...
class TestParcelsCalcMulti:
    @testit.workItemIds(35915)
    @testit.externalId('TestParcelsCalcMulti_35915')
    @testit.displayName('[200] POST /parcels/calc/multi - граничные значения "declaredPrice.value"')
    @testit.nameSpace('API')
    @testit.className('Parcels')
    @allure.epic('MobApp ')
    @allure.feature('API')
    @allure.story('API: Parcels')
    @allure.title('[200] POST /parcels/calc/multi - граничные значения "declaredPrice.value"')
    @allure.testcase(url='https://testit..ru/browse/35915')
    @pytest.mark.parametrize('declared_price', [1000.00, 200000.00])
    def test_parcels_calc_multi_with_declared_price_boundary_values(self, declared_price, with_auth):
        token = with_auth
        json_request = copy.deepcopy(parcel_calc_multi_with_custom_package_json)
        json_request['parcel']['declaredPrice']['value'] = declared_price
