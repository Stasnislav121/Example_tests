...
class PrivateClientsFaqPage(PrivateClientsBasePage):
    page_element = ':text("Частным клиентам. Вопросы и ответы")'
    page_route = '/faq/chastnym-klientam-voprosy-i-otvety'
    css_group = '.faq__group'

    @staticmethod
    def open(page):
        return PrivateClientsFaqPage(page).open_url('/faq/chastnym-klientam-voprosy-i-otvety').check_page()
